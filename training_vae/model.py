...
from layers import get_layer, param_init_fflayer, fflayer, param_init_gru, gru_layer, param_init_vae, vae_layer
...
def init_params(options):
    ...
    # Variational layer
    params = get_layer(options['vae'])[0](options, params, prefix='vae', 
                                           nhid=options['vae_nhid'], nlatent=options['vae_nlatent'],
                                           ndim=options['dim'])
...
def build_model(tparams, options):
    ...
    # Put encoded sentence through VAE
    proj_vae, mu, log_sigma = get_layer(options['vae'])[1](tparams, dec_ctx, options, prefix='vae')
    ...
    # decoder (ahead)
    projf = get_layer(options['decoder'])[1](tparams, embf, proj_vae, options,
                                             prefix='decoder_f',
                                             mask=y_mask)

    # decoder (behind)
    projb = get_layer(options['decoder'])[1](tparams, embb, proj_vae, options,
                                             prefix='decoder_b',
                                             mask=z_mask)
