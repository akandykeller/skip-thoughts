...
def build_model(tparams, options):
    ...
    # Word embedding (current)
    embc = tparams['Wemb'][x.flatten()].reshape([n_timesteps, n_samples, options['dim_word']])
    embc_shifted = tensor.zeros_like(embc)
    embc_shifted = tensor.set_subtensor(embc_shifted[1:], embc[:-1])
    embc = embc_shifted
    ...
    # decoder (current)
    projc = get_layer(options['decoder'])[1](tparams, embc, dec_ctx, options,
                                             prefix='decoder_c',
                                             mask=x_mask)
    ...
    # compute word probabilities (current)
    logit = get_layer('ff')[1](tparams, projc[0], options, prefix='ff_logit', activ='linear')
    logit_shp = logit.shape
    probs = tensor.nnet.softmax(logit.reshape([logit_shp[0]*logit_shp[1], logit_shp[2]]))
    ...
    costc = -tensor.log(probs.flatten()[x_flat_idx]+1e-8)
    costc = costc.reshape([x.shape[0],x.shape[1]])
    costc = (costc * x_mask).sum(0)
    costc = costc.sum()
